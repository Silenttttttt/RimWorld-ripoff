...
namespace rimworldripoff
{
   ...
   */
                ...
                if (board.Cells[xy.X, xy.Y].type == 0 || type == 0)
                {
                    ...
                    switch (type)
                    {
                        ...
                        case 1:
                            ...
                            board.Cells[xy.X, xy.Y].PartSize = new Size(board.CellSize - 1, board.CellSize);
                            ...
                            board.Cells[xy.X, xy.Y].counter = 0;
                            board.Cells[xy.X, xy.Y].speedtimer = 0;
                            board.Cells[xy.X, xy.Y].speeddelay = 1;
        ...
        /// <summary>
        ///check if part has too much velocity 
        ///and if part can go there
        ///

        public Point CellMovement(Point xy, int vx, int vy)
        {
            //  int temptick  = totalticks;
            // Point newxypos = xy;
            Point newxy = xy;
            Point vxy = new Point();
            ...
            //  {
            //  vxy = new Point();
            //newxy = new Point(xy.X + vx, xy.Y + vy);
            ...
            if (board.Cells[newxy.X + vxy.X, newxy.Y].Alive || board.Cells[newxy.X, newxy.Y + vxy.Y].Alive || board.Cells[newxy.X + vxy.X, newxy.Y + vxy.Y].Alive)
            {

                if (totalticks % 2 == 0)
                {

                    if (!board.Cells[xy.X + vxy.X, xy.Y].Alive && vxy.X != 0 && xy.X + vxy.X != 0 && xy.X + vxy.X != board.Height / board.CellSize - 1)
                    {
                        newxy.X = xy.X + vxy.X;

                        board.Cells[xy.X, xy.Y].vx = vx - Math.Sign(vx);//     vx = -Math.Sign(vx);
                    }
                    else if (!board.Cells[xy.X, xy.Y + vxy.Y].Alive && vxy.Y != 0 && xy.Y + vxy.Y != 0 && xy.Y + vxy.Y != board.Height / board.CellSize - 1)
                    {
                        newxy.Y = xy.Y + vxy.Y;
                        board.Cells[xy.X, xy.Y].vy = vy - Math.Sign(vy);//  vy = -Math.Sign(vy);
                    }
                    //   else if (!board.Cells[xy.X + vxy.X, xy.Y].Alive && )
                }
                else if (totalticks % 2 != 0)
                {
                    if (!board.Cells[xy.X, xy.Y + vxy.Y].Alive)
                    {
                        newxy.Y = xy.Y + vxy.Y;
                        board.Cells[xy.X, xy.Y].vy = vy - Math.Sign(vy);//   vy = -Math.Sign(vy);
                    }
                    else if (!board.Cells[xy.X + vxy.X, xy.Y].Alive && vxy.X != 0 && xy.X + vxy.X != 0 && xy.X + vxy.X != board.Width / board.CellSize - 1)
                    {
                        newxy.X = xy.X + vxy.X;
                        board.Cells[xy.X, xy.Y].vx = vx - Math.Sign(vx);
                    }
                    //   newxy.X = xy.Y - vy;
                }
                //  temptick++;
            ...
            {
                newxy = new Point(xy.X + vxy.X, xy.Y + vxy.Y);
                board.Cells[xy.X, xy.Y].vx -= vxy.X;
                board.Cells[xy.X, xy.Y].vy -= vxy.Y;
            }
